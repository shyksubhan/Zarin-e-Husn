...
const newNav = `<nav style="background: #fff; padding: 25px 0 15px 0;">
    <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 25px; padding: 0 40px; width: 100%; box-sizing: border-box;">
      ...
      <div class="nav-center" style="width: 33.33%; text-align: center;">
        <a href="/" style="text-decoration: none; color: #111; font-family: 'Amiri', serif; font-size: 3.5rem; font-weight: 700; line-height: 1; display: inline-block; white-space: nowrap;">
          ${urduLogo}
        </a>
      </div>
...
files.forEach(file => {
  ...
  if (fs.existsSync(filepath)) {
    ...
    // Replace existing nav entirely
    content = content.replace(/<nav style="background: #fff; padding: 15px 0;">[\s\S]*?<\/nav>/, newNav);
    
    fs.writeFileSync(filepath, content);
  }
});
console.log('Fixed navbar final');
