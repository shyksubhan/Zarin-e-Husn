/* ============================================================
   GOLNISÀ — Newsletter Routes (uses shared store)
   ============================================================ */
...
router.post('/', async (req, res) => {
  try {
    ...
    if (isFirebaseAvailable()) {
      ...
      if (!existing.empty) return res.json({ message: "You're already part of the Golnisà Circle! 💛", alreadySubscribed: true });
      ...
      return res.status(201).json({ message: "Welcome to Golnisà! 💛 You'll be the first to hear about new arrivals and offers.", id: ref.id });
    }

    /* In-memory via shared store */
    if (store.subscribers.find(s => s.email === normalEmail)) {
      return res.json({ message: "You're already part of the Golnisà Circle! 💛", alreadySubscribed: true });
    }
    ...
    sendNewsletterWelcome(normalEmail).catch(e => console.error('Welcome email failed:', e.message));
    return res.status(201).json({ message: "Welcome to Golnisà! 💛 You'll be the first to hear about new arrivals and offers.", id: sub.id });
