...
emailContent = emailContent.replace(
  /\$\{i\.name\}/g,
  "${i.name} ${i.variant && i.variant !== 'Standard' ? `(${i.variant})` : ''}"
);
...
adminContent = adminContent.replace(
  /\$\{i\.name\}/g,
  "${i.name} ${i.variant && i.variant !== 'Standard' ? `<span style=\"color:var(--gold);font-size:0.75rem;margin-left:4px;\">(${i.variant})</span>` : ''}"
);
...
console.log('Fixed variant visibility everywhere using simple replacements');
